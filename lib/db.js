...
const fetch = (room) => {
  ...
  return pool.query(`SELECT * FROM ${room} ORDER BY ID DESC`)
    .then((res) => {
...
const create = () =>
  pool.query("INSERT INTO sessions(title, created) VALUES ('untitled session', CURRENT_TIMESTAMP)")
    .then(getId)
    .then(() => fetch('sessions'))
    .catch(err => error(err));
...
const add = ({ room, title }) =>
  update(`INSERT INTO ${room}(topic, red, amber, green) VALUES ('${title}', 0, 0, 0)`, room);
