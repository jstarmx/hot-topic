...
const getTitle = room =>
  pool
    .query(`SELECT * FROM sessions WHERE id = ${roomId(room)}`)
...
const newSession = id =>
  pool
    .query(
      `CREATE TABLE session${id} (id SERIAL, topic TEXT, red INTEGER, amber INTEGER, green INTEGER)`
    )
    .then(() => id)
    .catch(err => error(err));
...
const create = () =>
  pool
    .query(
      "INSERT INTO sessions(title, created) VALUES ('untitled session', CURRENT_TIMESTAMP)"
    )
    .then(getId)
    .then(() => fetch('sessions'))
    .catch(err => error(err));
...
const add = ({ room, title }) =>
  update(
    `INSERT INTO ${room}(topic, red, amber, green) VALUES ('${title}', 0, 0, 0)`,
    room
  );
...
module.exports = { fetch, create, destroy, rename, add, remove, vote };
